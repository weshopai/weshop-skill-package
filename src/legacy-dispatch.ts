...
export class LegacyDispatchError extends Error {}
const general = new Set(["generate-image", "edit-image", "generate-video", "animate-image"]);
export function dispatchLegacyNaturalLanguage(text: string, overrides: Partial<RouteRequest> = {}): RoutePlan {
  const intent = { ...compileIntent(text), ...overrides };
  return dispatchLegacyRequest(intent);
}
export function dispatchLegacyRequest(request: RouteRequest & Pick<IntentCard, "raw" | "confidence" | "requiresResearch">): RoutePlan {
  if (isUnsupportedAudioRequest(request.raw)) throw new LegacyDispatchError("WeShop currently has no verified standalone audio-generation or audio-processing model.");
  ...
  if (request.operation === "character-sheet" || request.operation === "create-character") {
    ...
    if (request.requestedModel && request.requestedModel !== "gpt-image") throw new LegacyDispatchError("Final Character Reference Sheets require GPT Image 2 Medium/2K.");
    const model = getModel("gpt-image");
    if (!model || model.media !== "image") throw new LegacyDispatchError("Character reference sheets require an image model.");
  ...
  if (request.operation === "make-mugshot-photo") {
    if (!request.assets.includes("image")) throw new LegacyDispatchError("Make Mugshot Photo requires an authorized adult portrait image.");
    if (request.requestedModel && request.requestedModel !== "gpt-image") {
      throw new LegacyDispatchError("Make Mugshot Photo production requires GPT Image 2 Medium.");
    }
    const model = getModel("gpt-image");
    if (!model || model.media !== "image") throw new LegacyDispatchError("Make Mugshot Photo requires GPT Image 2.");
    return plan(request, 1, 1, {
      model,
  ...
  if (request.operation === "try-on") {
    if (!request.assets.includes("garment")) throw new LegacyDispatchError("Virtual Try-On requires: garment.");
    const model = request.requestedModel ? getModel(request.requestedModel) : getModel("gpt-image");
    if (!model || model.media !== "image") throw new LegacyDispatchError("Virtual Try-On requires an image model.");
  ...
  if (request.operation === "outfit-design") {
    if (!request.assets.includes("image")) throw new LegacyDispatchError("Outfit Design requires one person image.");
    const agent = agentRegistry.find((candidate) => candidate.name === "outfit-generator");
    if (!agent) throw new LegacyDispatchError("No registered Outfit Generator agent.");
    return plan(request, runs, batchCount, {
      agent,
  ...
  if (request.operation === "replace-model-or-scene" && request.assets.includes("model-reference")) {
    const model = request.requestedModel ? getModel(request.requestedModel) : getModel("gpt-image");
    if (!model || model.media !== "image") throw new LegacyDispatchError("Fashion Model Replacement requires an image model.");
    return plan(request, runs, batchCount, {
      model,
      category: "model-command",
      workflow: {
  ...
  if (request.operation === "change-pose") {
    if (!request.assets.includes("dressed-model")) throw new LegacyDispatchError("Change Pose requires: dressed-model.");
    ...
    if (!agent) throw new LegacyDispatchError("No registered AI Pose agent.");
  ...
  if (request.operation === "product-scene") {
    if (!request.assets.includes("product")) throw new LegacyDispatchError("AI Product requires: product.");
    const agent = agentRegistry.find((candidate) => candidate.name === "aiproduct");
    if (!agent) throw new LegacyDispatchError("No registered AI Product agent.");
    return plan(request, runs, batchCount, {
      agent,
      category: "commercial-workflow",
      workflow: {
  ...
  if (request.operation === "remove-background") {
    if (!request.assets.includes("image")) throw new LegacyDispatchError("Remove Background requires: image.");
    const agent = agentRegistry.find((candidate) => candidate.name === "removeBG");
    if (!agent) throw new LegacyDispatchError("No registered Remove Background agent.");
    return plan(request, 1, 1, {
      agent,
      category: "commercial-workflow",
      workflow: {
  ...
  if (request.operation === "expand-image") {
    if (!request.assets.includes("image")) throw new LegacyDispatchError("Expand Image requires: image.");
    const model = request.requestedModel ? getModel(request.requestedModel) : getModel("gpt-image");
    if (!model || model.media !== "image") throw new LegacyDispatchError("Expand Image requires an image model.");
    return plan(request, 1, 1, {
      model,
      category: "model-command",
      workflow: {
  ...
  if (request.operation === "create-animal") {
    ...
    if (request.requestedModel && request.requestedModel !== modelId) {
      throw new LegacyDispatchError(`create-animal production requires ${modelId} for this brief.`);
    }
    const model = getModel(modelId);
    if (!model || model.media !== "image") throw new LegacyDispatchError("create-animal requires a cataloged image model.");
    return plan(request, 1, 1, {
      model,
      category: "model-command",
  ...
  if (["create-logo", "create-flag", "restyle-room", "preview-landscape", "make-flowchart"].includes(request.operation)) {
    ...
    if ((request.operation === "restyle-room" || request.operation === "preview-landscape") && !request.assets.includes("image")) {
      throw new LegacyDispatchError(`${selected.workflow} requires one source image.`);
    }
    if (request.requestedModel && request.requestedModel !== selected.modelId) {
      throw new LegacyDispatchError(`${selected.workflow} production requires ${selected.modelId}.`);
    }
    const model = getModel(selected.modelId);
    if (!model || model.media !== "image") throw new LegacyDispatchError(`${selected.workflow} requires a cataloged image model.`);
    return plan(request, 1, 1, {
      model,
      category: "model-command",
      workflow: {
  ...
  if (general.has(request.operation)) {
    ...
    if (!model) throw new LegacyDispatchError(`No ${request.media} model matches this request.`);
    if (model.media !== request.media) throw new LegacyDispatchError(`${model.id} is a ${model.media} model, not ${request.media}.`);
  ...
  if (!agent) throw new LegacyDispatchError(`No registered agent for ${request.operation}.`);
  if (!agent.requiredAssets.every((asset) => request.assets.includes(asset))) throw new LegacyDispatchError(`${agent.name} requires: ${agent.requiredAssets.join(", ")}.`);
