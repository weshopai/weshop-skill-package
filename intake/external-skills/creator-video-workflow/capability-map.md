# WeShop capability substitution: creator-video-workflow

| External behavior | Inputs and constraints | WeShop decision | Evidence |
| --- | --- | --- | --- |
| A disabled source workflow; active creator-native outcome is separately assessed. | Authorized assets/facts, explicit preservation constraints, and observable acceptance | Reject disabled route; use only a current verified route if promoted | Source entrypoint review and current Router contract |
...
- Adjacent current ownership: `make-koc-video`.
