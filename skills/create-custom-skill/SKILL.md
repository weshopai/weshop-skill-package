...
3. Create an isolated draft at `~/.weshop-skill-package/custom-skills/<slug>/` unless the user names another draft root. Use lowercase kebab-case and refuse to overwrite an existing directory without explicit permission. When working from this package checkout, `npm run skills:custom:init -- <slug>` may create the initial `SKILL.md` and `intake.md` worksheets.
...
9. Run the mechanical custom review when the package checkout is available: `npm run skills:custom:review -- <draft-directory>`. Repair structural blockers, but do not treat the mechanical result as independent semantic approval.
