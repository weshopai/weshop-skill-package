...
test("routes character creation to eight canonical-first GPT Image tasks", () => { const plan = routeNaturalLanguage("创建一个可以持续用于漫画的原创角色"); assert.equal(plan.workflow?.id, "create-character"); assert.equal(plan.model?.id, "gpt-image"); assert.equal(plan.runs, 8); assert.equal(plan.batchCount, 1); assert.equal(plan.params.batchCount, 1); assert.equal(plan.params.executionOrder, "canonical-first-then-derived"); assert.deepEqual(plan.params.tasks, ["canonical-design-sheet", "full-body-front", "full-body-back", "head-close-up", "lighting-study", "final-look-portrait", "scene-1", "scene-2"]); });
test("uses GPT Image 2 Medium/2K and canonical binding when a character reference exists", () => { const plan = routeNaturalLanguage("根据这张图片制作角色设定图", { assets: ["model-reference"] }); assert.equal(plan.workflow?.id, "create-character"); assert.equal(plan.model?.id, "gpt-image"); assert.equal(plan.params.quality, "medium"); assert.equal(plan.params.imageSize, "2K"); assert.match(String(plan.params.canonicalReferenceBinding), /tasks 2-8/); });
