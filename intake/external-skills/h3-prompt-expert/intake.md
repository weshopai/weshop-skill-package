# External Skill intake: h3-prompt-expert
...
- User-visible outcomes: 视频想法与参考资产的结构化提示词
- Required inputs: 视频意图、资产角色、目标
...
| --- | ---: | --- | --- | --- | --- |
| `generate-video` | 0.68 | Adjacent delivery | 模型无关的原子视频执行简报 | The narrower existing result is requested | Use only after the candidate plan is accepted |

- Proposed frontmatter distinction: 模型无关的原子视频执行简报
...
| --- | --- | --- |
| 做一个视频想法与参考资产的结构化提示词 | h3-prompt-expert | Candidate outcome |
| 用我的素材完成视频想法与参考资产的结构化提示词 | h3-prompt-expert | Input and preservation contract |
| 先规划再交付视频想法与参考资产的结构化提示词 | h3-prompt-expert | Candidate workflow |
...
- Unsafe or removed behavior: 官方 H3 格式、Hub、自动执行
