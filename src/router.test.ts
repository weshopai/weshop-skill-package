...
test("keeps image and video catalogs separate", () => { assert.equal(listModels("image").length, 8); assert.equal(listModels("video").length, 11); });
test("routes character creation to one canonical task before an optional expansion gate", () => { const plan = routeNaturalLanguage("创建一个可以持续用于漫画的原创角色"); assert.equal(plan.workflow?.id, "create-character"); assert.equal(plan.model?.id, "gpt-image"); assert.equal(plan.runs, 1); assert.equal(plan.batchCount, 1); assert.equal(plan.params.batchCount, 1); assert.equal(plan.params.executionOrder, "canonical-qa-confirmation-gate-then-optional-seven-task-expansion"); assert.deepEqual(plan.params.defaultTasks, ["canonical-design-sheet"]); assert.deepEqual(plan.params.optionalConfirmedTasks, ["full-body-front", "full-body-back", "head-close-up", "lighting-study", "final-look-portrait", "scene-1", "scene-2"]); assert.equal(plan.params.expansionRequiresPostQaUserConfirmation, true); });
