...
import { compileIntent, compileKlingPrompt, getModel, listModels, modelsWithCapability, route, routeNaturalLanguage } from "./index.js";
...
test("uses one sampled final QA gate for ordinary routes", () => {
  const plan = routeNaturalLanguage("生成4张海边小屋插画");
...
test("uses targeted sampled QA for preservation-sensitive routes", () => {
  const plan = routeNaturalLanguage("用这件服装生成4张模特试穿图，保持 logo 一致", { assets: ["garment"] });
...
test("reserves strict per-output QA for hard output and safety contracts", () => {
  const transparent = routeNaturalLanguage("去掉这张图的背景，给我透明 PNG", { assets: ["image"] });
  const fictional = routeNaturalLanguage("Create a fictional mugshot from this authorized adult photo", { assets: ["image"] });
...
test("routes try-on through GPT Image 2 without progressive model trials", () => {
  const fast = routeNaturalLanguage("用这件服装生成1张模特试穿图", { assets: ["garment"] });
...
test("routes outfit invention separately from virtual try-on", () => {
  const plan = routeNaturalLanguage("给这个人做一套穿搭设计，改成建筑感都市剪裁", { assets: ["image"] });
...
test("routes a supplied replacement-model reference through GPT Image 2", () => {
  const plan = routeNaturalLanguage("替换这张服装图里的模特，保持衣服不变", { assets: ["dressed-model", "model-reference"] });
...
test("keeps the background for a text-described fashion model replacement", () => {
  const plan = routeNaturalLanguage("替换这张服装图里的模特，保持背景不变", { assets: ["dressed-model"] });
...
test("returns three independent pro pose choices for an underspecified pose request", () => {
  const plan = routeNaturalLanguage("给这个模特换一个姿势", { assets: ["dressed-model"] });
...
test("returns one pro run when the user defines a pose", () => {
  const plan = routeNaturalLanguage("把模特姿势换成侧身站立，一只手放在腰间", { assets: ["dressed-model"] });
...
test("routes a real product scene through a generated custom location", () => {
  const plan = routeNaturalLanguage("把这个商品放到温暖的木质棚拍场景里", { assets: ["product"] });
...
test("routes background removal to a true transparent PNG with edge QA", () => {
  const plan = routeNaturalLanguage("去掉这张图的背景，给我透明 PNG", { assets: ["image"] });
...
test("routes ratio correction through GPT Image 2 with one Expand Image fallback", () => {
  const plan = routeNaturalLanguage("把这张图片扩图到 4:5，保持主体居中", { assets: ["image"] });
...
test("routes routine final image editing to GPT Image 2 Medium/2K", () => { const plan = routeNaturalLanguage("编辑这张图片，替换背景", { assets: ["image"] }); assert.equal(plan.model?.id, "gpt-image"); assert.equal(plan.params.quality, "medium"); assert.equal(plan.params.imageSize, "2K"); assert.equal(plan.category, "model-command"); });
test("uses Nano 2 only for explicit draft divergence", () => { const plan = routeNaturalLanguage("编辑这张图片，做三种快速草稿分叉", { assets: ["image"] }); assert.equal(plan.model?.id, "nano-banana-edit"); assert.equal(plan.params.modelName, "nano2"); });
test("uses Nano Pro only for explicit internal convergence", () => { const plan = routeNaturalLanguage("编辑这张图片用于内部评审和内部收敛", { assets: ["image"] }); assert.equal(plan.model?.id, "nano-banana-edit"); assert.equal(plan.params.modelName, "nano"); assert.equal(plan.params.imageSize, "2K"); });
test("rejects a routine final edit explicitly forced to Nano", () => { assert.throws(() => routeNaturalLanguage("编辑这张图片用于最终交付", { assets: ["image"], requestedModel: "nano-banana-edit" }), /reserved for explicit draft divergence/); });
test("routes readable poster text to GPT Image 2 Medium", () => {
  const plan = routeNaturalLanguage("设计一张海报，标题写新品发布");
  assert.equal(plan.model?.id, "gpt-image");
  assert.equal(plan.params.quality, "medium");
...
test("routes every final Banner through GPT Image 2 even when text-free and artistic", () => {
  const plan = routeNaturalLanguage("设计一张无文字的实验艺术风格网站横幅");
  assert.equal(plan.model?.id, "gpt-image");
  assert.equal(plan.params.quality, "medium");
...
test("uses Midjourney only for an upstream Banner artistic-direction reference", () => {
  const plan = routeNaturalLanguage("为网站 Banner 生成一组无文字的艺术方向参考图");
...
test("keeps Banner direction references with readable copy on GPT Image 2", () => {
  const plan = routeNaturalLanguage("为网站 Banner 生成艺术方向参考图，标题写新品发布");
  assert.equal(plan.model?.id, "gpt-image");
  assert.equal(plan.params.quality, "medium");
});
test("rejects Midjourney as an explicitly requested final Banner model", () => {
  assert.throws(
    () => routeNaturalLanguage("用 Midjourney 制作最终无文字 Banner", { requestedModel: "midjourney" }),
...
test("routes pure artistic exploration to Midjourney and accounts for four outputs", () => {
  const plan = routeNaturalLanguage("创作一组实验性艺术插画");
  assert.equal(plan.model?.id, "midjourney");
  assert.equal(plan.params.outputCount, 4);
});
test("routes pure photorealistic text-to-image to Z-Image", () => {
  const plan = routeNaturalLanguage("生成一张写实摄影风格的海边小屋，不要文字");
...
test("routes the reviewed Atom Skills to direct models rather than Tools wrappers", () => {
  ...
  for (const [brief, assets, operation, modelId] of cases) {
    const plan = routeNaturalLanguage(brief, { assets: [...assets] });
...
test("uses GPT Image 2 Medium for a non-photographic animal brief", () => {
  const plan = routeNaturalLanguage("Create animal: an original friendly cloud creature in a flat editorial style");
  assert.equal(plan.intent.operation, "create-animal");
  assert.equal(plan.model?.id, "gpt-image");
  assert.equal(plan.params.quality, "medium");
});
test("requires source images for room and landscape preview routes", () => {
  assert.throws(() => routeNaturalLanguage("Restyle this room as warm Japandi", { assets: [] }), /requires one source image/);
  assert.throws(() => routeNaturalLanguage("Redesign this courtyard for a dry climate", { assets: [] }), /requires one source image/);
});
test("rejects model overrides that bypass reviewed Atom routes", () => {
  assert.throws(() => routeNaturalLanguage("Create logo for NORTHLINE COFFEE", { requestedModel: "midjourney" }), /requires gpt-image/);
  assert.throws(() => routeNaturalLanguage("Create animal: a photorealistic red panda", { requestedModel: "gpt-image" }), /requires z-image/);
});
test("excludes text-to-image-only models when an image input exists", () => {
  assert.throws(() => routeNaturalLanguage("编辑这张图片", { assets: ["image"], requestedModel: "z-image" }), /text-to-image only/);
});
test("routes demanding commercial lighting to Seedream", () => {
  const plan = routeNaturalLanguage("生成一张珠宝广告，强调精细棚拍光线和材质塑造");
  assert.equal(plan.model?.id, "seedream");
});
test("routes image animation to Kling V3", () => { const plan = routeNaturalLanguage("让这张图片动起来，生成一个视频", { assets: ["image"] }); assert.equal(plan.model?.id, "kling"); assert.equal(plan.model?.media, "video"); assert.equal(plan.params.modelName, "Kling_3_0"); });
test("routes large-amplitude motion to MiniMax H3", () => { const plan = routeNaturalLanguage("生成一段高动态奔跑和跳跃的视频"); assert.equal(plan.model?.id, "minimax-h3"); });
test("routes complex multimodal references to Kling V3 Omni", () => { const plan = routeNaturalLanguage("用这些图片做一段多图参考视频，还要参考这段视频的镜头运动"); assert.equal(plan.model?.id, "kling-v3-omni"); });
test("routes audio-visual artistic work to Seedance 2.5", () => { const plan = routeNaturalLanguage("生成一段音乐卡点的艺术短片，要求音画同步"); assert.equal(plan.model?.id, "seedance-2-5"); });
test("routes source-image premium synchronous dialogue to Veo", () => { const plan = routeNaturalLanguage("用这张图片生成电影级对话视频，要求同步对话和精准音效"); assert.equal(plan.model?.id, "veo-ai"); });
...
test("routes character creation to one canonical task before an optional parallel expansion wave", () => { const plan = routeNaturalLanguage("创建一个可以持续用于漫画的原创角色"); assert.equal(plan.workflow?.id, "create-character"); assert.equal(plan.model?.id, "gpt-image"); assert.equal(plan.runs, 1); assert.equal(plan.batchCount, 1); assert.equal(plan.params.batchCount, 1); assert.equal(plan.params.executionOrder, "canonical-qa-confirmation-gate-then-optional-seven-task-expansion"); assert.deepEqual(plan.params.defaultTasks, ["canonical-design-sheet"]); assert.deepEqual(plan.params.optionalConfirmedTasks, ["full-body-front", "full-body-back", "head-close-up", "lighting-study", "final-look-portrait", "scene-1", "scene-2"]); assert.equal(plan.params.expansionRequiresPostQaUserConfirmation, true); assert.deepEqual(plan.params.expansionSubmission, { mode: "parallel-wave", concurrency: 7, awaitBetweenSubmissions: false, prepareAllPayloadsAndKeysBeforeFirstSubmit: true, batchCountPerTask: 1 }); });
test("uses GPT Image 2 Medium/2K and requires both reference-image fields for confirmed expansion", () => { const plan = routeNaturalLanguage("根据这张图片制作角色设定图", { assets: ["model-reference"] }); assert.equal(plan.workflow?.id, "create-character"); assert.equal(plan.model?.id, "gpt-image"); assert.equal(plan.params.quality, "medium"); assert.equal(plan.params.imageSize, "2K"); assert.deepEqual(plan.params.canonicalReferenceBinding, { source: "canonical-design-sheet.result.image", requiredIn: ["input.images", "params.images"], appliesTo: "all optional confirmed tasks" }); assert.deepEqual(plan.params.canonicalReferenceRecovery, { lookup: "task-1 operationKey then exact executionId", extract: "data.executions[*].result[*].image", persistAs: "canonicalImageUrl", repairPayloads: true, regenerateTask1: false, derivedSubmissionBeforeRecovery: false }); });
test("routes a fictional mugshot-style portrait directly to GPT Image 2", () => {
  const plan = routeNaturalLanguage("把这张获授权成年人照片做成明确标注 FICTIONAL 的虚构 mugshot 风格照片", { assets: ["image"] });
...
test("rejects a non-GPT model override for make-mugshot-photo", () => {
  assert.throws(
    () => routeNaturalLanguage("Create a fictional mugshot from this authorized adult photo", { assets: ["image"], requestedModel: "nano-banana-edit" }),
    /requires GPT Image 2 Medium/
  );
});
test("rejects unsupported standalone audio generation instead of inventing a route", () => {
  assert.throws(
    () => routeNaturalLanguage("生成一段 12 秒的原创环境音频，不要画面"),
    /no verified standalone audio-generation or audio-processing model/
  );
});
test("rejects unsupported standalone audio through the structured Router entrypoint", () => {
  assert.throws(
    () => route({ ...compileIntent("generate audio file"), raw: "generate audio file", confidence: 1, requiresResearch: false }),
    /no verified standalone audio-generation or audio-processing model/
  );
});
test("keeps native audio inside a requested video on the video route", () => {
  const plan = routeNaturalLanguage("生成一段带自然环境声的雨天短片");
...
test("adds Kling 3.0 atomic execution and error-aware retry parameters", () => {
  const plan = routeNaturalLanguage("让这张图片动起来，镜头缓慢推进", { assets: ["image"] });
