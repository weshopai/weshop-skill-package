# WeShop capability substitution: anime-game-pv

| External behavior | Inputs and constraints | WeShop decision | Evidence |
| --- | --- | --- | --- |
| A bounded anime/game-character promotional video with identity, world, motion, and deterministic exact-copy handoffs. | Authorized assets/facts, explicit preservation constraints, and observable acceptance | Split candidate: `make-anime-game-pv`; use only a current verified route if promoted | Source entrypoint review and current Router contract |
...
- Split candidate: `make-anime-game-pv`.
- Adjacent current ownership: `make-music-video`, `generate-video`, `create-character`.
