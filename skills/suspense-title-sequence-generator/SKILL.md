...
4. Generate and inspect each approved keyframe with a current text-safe image route. Require one Runtime-tracked submission per keyframe; use an `operationKey` only when exposed, require `executionId`, poll terminal state, and regenerate only a failing keyframe with a Runtime-linked revised submission. If text is unreliable, use deterministic supplied type instead of accepting garbling.
5. Compile one coherent 15-second motion-graphics sequence from the six accepted anchors. Select a current video route suited to the approved references; require one Runtime-tracked submission, terminal polling, and no blind retry. Preserve graphic panels, masks, transitions, supplied credits, and final title hold.
