...
Provide authorized music and visual files, destination, ratio, target duration or song section, must-use/exclude clips, desired energy curve, and named effects.

#### Example

```text
Cut these eight licensed travel clips into a 30-second 9:16 reel using the supplied chorus. Let the first phrase breathe, accelerate toward the drop, use one restrained white-flash accent, and preserve the original framing of faces.
```

## Workflow

1. Inventory source paths, ownership/permission, codecs, dimensions, frame rates, duration, audio, and requested deliverables. Never fetch media merely because a reference is online.
...
5. Build an inspectable EDL with stable IDs, source in/out, timeline in/out, transition, speed, crop, audio rule, beat/phrase rationale, and acceptance note. Confirm total duration, gaps, overlaps, repeated source ranges, and rights before rendering.
...
- Content per video: User-authorized footage/stills synchronized to supplied music
