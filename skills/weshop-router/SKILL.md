...
3. Before planning steps, record a `planning` decision: `shape` is `single-atom` only when one independent Atom can deliver the complete requested result, otherwise `multi-step`; `reason` is one of `single_atomic`, `dependency_chain`, `ambiguity`, `research`, or `risk`; and `clarificationRequired` is true only when one focused question must be answered before execution. A single-atom decision goes directly to one Skill node. A multi-step decision is the condition for Router composition, not a generic preface for every request.
...
6. If several Atoms are required, define one final acceptance contract. Do not repeat QA after every Atom: preserve handoff invariants and run one final gate after the last generative step. A recurring composition may later become its own Atom when it still produces one clear result.
...
10. Read [references/adaptive-planning.md](references/adaptive-planning.md) for the intent and DAG schema whenever a request is compound, research-dependent, or materially ambiguous.
11. After Skill selection, let the selected Atom own its agent/model route. Apply the shared root `model-selection-policy.md` only where the Atom leaves a model choice open. Match media and verified capabilities; `unknown` is unverified. After choosing the executable model, read exactly its guide from [references/model-prompt-routing.md](references/model-prompt-routing.md) and shape `textDescription` without importing fields that the live WeShop schema does not expose.
