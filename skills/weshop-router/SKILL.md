...
11. For execution, prefer a native WeShop tool exposed by the harness. Otherwise, when local command execution is available, use the official `weshop` CLI when `weshop --version` succeeds; only use the package's built-in `weshop-skill` command when the official executable is absent. Follow the backend-specific syntax and failure rules in [references/built-in-cli.md](references/built-in-cli.md). Authentication, validation, or transport errors are not reasons to switch backends.
