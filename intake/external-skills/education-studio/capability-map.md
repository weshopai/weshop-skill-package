# WeShop capability substitution: education-studio

| External behavior | Inputs and constraints | WeShop decision | Evidence |
| --- | --- | --- | --- |
| Teaching and explainer outcomes based on user-owned educational facts and materials. | Authorized assets/facts, explicit preservation constraints, and observable acceptance | Differentiate to `make-explainer-video`; use only a current verified route if promoted | Source entrypoint review and current Router contract |
...
- Differentiate to `make-explainer-video`.
- Adjacent current ownership: `make-explainer-video`, `create-image-deck`.
