...
5. Require the Runtime to track one stable submission identity per page, require a non-empty `executionId`, and poll each accepted run to terminal state. Use an `operationKey` only when exposed by the harness; do not create a carousel-owned ledger. Pages with no dependency may run in parallel only when each operation is independently tracked.
