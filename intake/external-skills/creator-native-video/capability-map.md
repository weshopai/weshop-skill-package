# WeShop capability substitution: creator-native-video

| External behavior | Inputs and constraints | WeShop decision | Evidence |
| --- | --- | --- | --- |
| Creator-native social commercial content, distinguished from official brand campaigns and product-benefit commercials. | Authorized assets/facts, explicit preservation constraints, and observable acceptance | Split candidate: `make-koc-video`; use only a current verified route if promoted | Source entrypoint review and current Router contract |
...
- Split candidate: `make-koc-video`.
- Adjacent current ownership: `make-product-commercial`, `brand-promo-video-generator`.
