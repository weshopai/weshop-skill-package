# WeShop capability substitution: cinematic-title-sequence

| External behavior | Inputs and constraints | WeShop decision | Evidence |
| --- | --- | --- | --- |
| A title, credits, or film-teaser sequence whose supplied exact copy and world continuity are the deliverable. | Authorized assets/facts, explicit preservation constraints, and observable acceptance | Split candidate: `make-cinematic-title-sequence`; use only a current verified route if promoted | Source entrypoint review and current Router contract |
...
- Split candidate: `make-cinematic-title-sequence`.
- Adjacent current ownership: `make-kinetic-typography`, `make-video-intro`, `generate-video`.
