...
10. After Skill selection, let the selected Atom own its agent/model route. Apply the shared root `model-selection-policy.md` only where the Atom leaves a model choice open. Match media and verified capabilities; `unknown` is unverified.
11. For execution, prefer a native WeShop OpenAPI tool exposed by the harness. Otherwise, when local command execution is available, use the package's built-in `weshop-skill` command described in [references/built-in-cli.md](references/built-in-cli.md). It supports Standard and Premium Agents without requiring the separate official CLI.
