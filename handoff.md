...
## 2026-08-21: MiniMax featured video-skill intake

- Inspected both files (`SKILL.md`, `meta.yaml`) in each supplied MiniMax Design export and recorded immutable SHA-256 provenance plus product decomposition, security boundaries, routing comparisons, and per-operation WeShop mappings under `intake/external-skills/`.
- Retained `fpv-tour-video-generator` and `pov-short-film-generator` as distinct complete-outcome Atom candidates (`make-fpv-tour-video`, `make-pov-short-film`), pending live-schema and representative-output validation. FPV owns continuous spatial route/landmark coverage; POV owns subjective-camera narrative grammar.
- Correction: the project rule is to preserve every supplied coherent external Skill as an independent candidate and distinguish it from adjacent candidates; no source outcome is merged into an existing Atom during intake. The three initially misclassified records are now independent candidates: `minimalist-product-ad-generator`, `music-video-subtitle-generator`, and `suspense-title-sequence-generator`.
- Audited the scoring mechanism: relationship scores in intake are static, manually reasoned adjacency metadata; they do not select a Skill or permit fusion. Runtime selection uses request-specific `intentMatchScore`, and `src/adaptive-router.ts` rejects a selected candidate below the maximum. Added a five-factor static-score calibration (outcome, inputs, preservation, deliverable, exclusions) and defined score bands in the maintainer SOP and intake template.

## 2026-08-22: promote five independently intaked video Atoms

- Promoted all five supplied external outcomes as independently installable Atoms: `fpv-tour-video-generator`, `minimalist-product-ad-generator`, `music-video-subtitle-generator`, `pov-short-film-generator`, and `suspense-title-sequence-generator`. None is merged into an existing Atom; adjacent Skills are named only for Router comparison and accepted-artifact handoffs.
- Each Atom has a clear output contract, source-role/preservation rules, current catalog-based model policy, stable operation key and terminal-polling rule, targeted acceptance checks, source-provenance reference, and explicit exclusions. The five promotion records now point to their packaged provenance files.
- No source MiniMax tool, cover media, dependency, provider field, or source prose was copied. No paid representative run has been authorized or performed; live output-specific parity remains a future acceptance gate.
- No MiniMax source instructions, tool bindings, cover assets, dependencies, model defaults, audio promises, automatic publishing, or retry behavior were copied. No paid generation, installable Atom change, commit, push, or release was performed.
