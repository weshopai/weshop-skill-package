...
test("routes a new character sheet to the WeShop-native workflow", () => { const plan = routeNaturalLanguage("生成一个电影角色三视图和近景设定图"); assert.equal(plan.workflow?.id, "character-reference-sheet"); assert.equal(plan.model?.id, "gpt-image"); assert.equal(plan.params.explicitMultiPanel, true); });
test("uses GPT Image 2 Medium/2K when a character reference exists", () => { const plan = routeNaturalLanguage("根据这张图片制作角色设定图", { assets: ["model-reference"] }); assert.equal(plan.model?.id, "gpt-image"); assert.equal(plan.params.quality, "medium"); assert.equal(plan.params.imageSize, "2K"); });
