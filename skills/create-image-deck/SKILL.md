...
5. Select one representative content page as the visual master. Generate it with GPT Image 2 as one Runtime-tracked submission, `batchCount: 1`, authorized references only, and the page's specific layout and preservation constraints. Use an operation key only when the harness exposes it. Poll to a terminal state and inspect it before continuing.
