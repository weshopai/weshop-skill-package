...
<p align="center">
  <img alt="85 Skills" src="https://img.shields.io/badge/Skills-85-7530FE?style=flat-square" />
...
> This repository contains **84 focused Atom Skills + 1 adaptive Router**. It also includes a standalone `weshop-skill` CLI, so the official WeShop CLI is not required.
...
The `skills/` directory contains 84 Atom Skills and one Router. Categories below are for browsing only and do not participate in hard-coded Router selection.
...
| Video | `add-video-effect`, `animate-image`, `combine-videos`, `correct-video-color`, `edit-social-video`, `generate-video`, `make-explainer-video`, `make-music-video`, `make-podcast-video`, `make-product-commercial`, `make-talking-video`, `make-video-intro`, `remove-video-mark`, `restyle-video`, `upscale-video` |
...
| `npm run models:routing-validate` | Validate model routes across all 84 Atom Skills |
