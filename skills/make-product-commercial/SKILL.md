...
5. Use the downstream route's native fields only. Require one Runtime-tracked submission per shot and an `executionId`; use an `operationKey` only when the harness exposes it, poll to terminal state, and download each terminal-success MP4. Do not create a workflow-owned ledger. Generate missing still assets through a suitable image Atom; do not ask a video model to repair a source product image implicitly.
