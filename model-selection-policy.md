...
- Demanding lighting/material rendering or Asian fashion/e-commerce aesthetics: consider Seedream 5 Pro. Use it only when one of those conditions exists, and never for readable text. Seedream 5 Lite is an explicit variant for 3K output, more than ten references, or its optional web-search enrichment; it is not a silent cost fallback.
...
- After model selection, load only the matching guide from `skills/orchestrate-multi-step-workflow/references/model-prompt-routing.md`. Guides shape prompts; the live WeShop schema alone authorizes parameters.
- Video: MiniMax H3 for large/natural human motion and H3 multimodal or first/last-frame modes; Kling V3 Omni for several image roles or one motion-reference video; Kling 3.0 for precise one/two-frame or product I2V; Seedance 2.5 for long multimodal, audiovisual, artistic, or talking performance; Veo 3.1 for one-source-image premium synchronous dialogue/SFX at 16:9 or 9:16; Seedance 2.0 for routine image-led video; Seedance 2.0 Mini only for an explicit low-cost draft, preview, or concept-validation route.
