...
6. Submit one Runtime-tracked operation per shot, require a non-empty `executionId`, poll to terminal state, and retain the exact accepted result. Use an `operationKey` only when the harness exposes it; do not create a workflow-owned ledger. Strip planning labels from generation prompts. Preserve character/scene anchors, one action, camera, start/end state, audio behavior, and forbidden additions.
