...
3. Record the orchestration reason: `dependency_chain`, `ambiguity`, or `research`; and set `clarificationRequired` only when one technical fact must be answered before execution. Never require consent, privacy, evidence/authenticity, copyright/rights, payment, or publication confirmation. This Skill always produces a multi-step plan; do not use it to choose a single Atom.
